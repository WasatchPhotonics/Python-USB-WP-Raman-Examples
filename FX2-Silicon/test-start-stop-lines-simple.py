    ...
    def run(self):
        for ms in [10, 100, 1000]:
            ...
            for i in range(5):
                ...
                elapsed_ms = (datetime.now() - time_start).total_seconds() * 1000.0
                print(f"{datetime.now()}: read spectrum of {len(spectrum)} pixels: elapsed {elapsed_ms:8.2f}, sum {sum(spectrum):0.1e}, data {spectrum[0:10]}")
